...
public class AuthService {
    ...
    /**
     * Realiza la autenticación de un usuario mediante username o email y contraseña.
     * <p>
     * El método busca al usuario por el valor proporcionado en {@code usernameOrEmail}, compara la
     * contraseña ingresada con la almacenada en la base de datos y, si es correcta, genera un token
     * JWT que será usado para autenticación en futuras solicitudes.
     * </p>
     *
     * @param credentials DTO que contiene las credenciales del usuario (username/email y password)
     * @return AuthResponseDTO que incluye el usuario autenticado y el token JWT generado
     ...
     * <ol>
     * <li>Verifica si ya existe un usuario con el mismo username y lanza una excepción si es
     * así.</li>
     * <li>Valida el formato del email proporcionado y lanza una excepción si no es válido.</li>
     * <li>Asigna un rol predeterminado al usuario (por ID en este caso).</li>
     * <li>Crea un objeto User con los datos proporcionados y algunos valores por defecto (activo,
     * no verificado, no baneado, etc.).</li>
     * <li>Guarda el usuario en la base de datos mediante {@code uService.saveUser}.</li>
     * <li>Genera un token JWT para el usuario recién creado.</li>
     * </ol>
     * </p>
     *
     * @param userData DTO que contiene los datos necesarios para registrar un usuario (nombre,
     *        apellido, username, email, contraseña, teléfono)
     * @return AuthResponseDTO que incluye el usuario registrado y el token JWT generado
     * @throws UserException si ya existe un usuario con el mismo username o si el email no es
     *         válido
     */
    public AuthResponseDTO register(RegisterRequestDTO userData) {
        ...
        User user = User.builder().name(userData.getName()).surname(userData.getSurname())
                .username(userData.getUsername()).email(userData.getEmail())
                .password(userData.getPassword()).phone(userData.getPhone()).role(role)
                .isActive(true).isVerified(false).isBanned(false).isDeleted(false)
